...
- Week README file: `WeekN/README.md` (create if missing)

## Procedure
1. Normalize week input using [week mapping](./references/week-and-question-normalization.md).
2. Open `WeekN/` and locate:
   - the week PDF (`*.pdf`)
   - the week README (`README.md` if exists, otherwise will be created)
3. Read the week PDF and extract **only programming/practical questions** (skip theoretical exercises) using the [PDF parsing rules](./references/pdf-parsing-rules.md).
...
- If PDF cannot be parsed: ask the user to provide the list of questions manually, or ask for the exercise text
...
- State whether README was created or updated
